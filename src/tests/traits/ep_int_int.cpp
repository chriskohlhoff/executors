...
{
  std::experimental::async_completion<CompletionToken, void(std::exception_ptr, int, int)> completion(tok);
  ...
  try
  {
    ...
    std::move(completion.completion_handler)(std::exception_ptr(), 1, 2);
  }
  catch (...)
  {
    std::move(completion.completion_handler)(std::current_exception(), 1, 2);
  }
...
int main()
{
  ...
  std::future<std::tuple<int, int>> f1 = async_foo(false, std::experimental::use_future);
  try
  {
    f1.get();
    ++success_count;
  ...
  std::future<std::tuple<int, int>> f2 = async_foo(true, std::experimental::use_future);
  try
  {
    f2.get();
    ++success_count;
