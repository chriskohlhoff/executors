...
{
  std::experimental::async_completion<CompletionToken, void(std::exception_ptr, int)> completion(tok);
  ...
  try
  {
    ...
    completion.handler(std::exception_ptr(), 1);
  }
  catch (...)
  {
    completion.handler(std::current_exception(), 1);
  }
...
void handler1(const std::exception_ptr& e, int)
{
...
{
  handler2() {}
  void operator()(const std::exception_ptr& e, int)
  {
    e ? ++fail_count : ++success_count;
  }
};
...
int main()
{
  ...
  async_foo(false, [](std::exception_ptr e, int){ e ? ++fail_count : ++success_count; });
  async_foo(true, [](std::exception_ptr e, int){ e ? ++fail_count : ++success_count; });
