...
struct __dispatch_action
{
  ...
  {
    __e.dispatch(forward<_Handler>(__h), __a);
  }
};
...
auto dispatch_at(const chrono::time_point<_Clock, _Duration>& __abs_time, _CompletionToken&& __token)
{
  ...
  typedef __timer_op<_Clock, associated_executor_t<_Handler>, __dispatch_action, _Handler> _Op;
...
inline auto dispatch_at(const chrono::time_point<_Clock, _Duration>& __abs_time,
  const _Executor& __e, _CompletionToken&& __token,
    typename enable_if<is_executor<_Executor>::value>::type*)
{
  typedef typename handler_type<_CompletionToken, void()>::type _Handler;
  typedef __timer_op<_Clock, _Executor, __dispatch_action, __work_dispatcher<_Handler>> _Op;
  async_completion<_CompletionToken, void()> __completion(__token);
...
{
  return (dispatch_at)(__abs_time, __c.get_executor(), forward<_CompletionToken>(__token));
}
