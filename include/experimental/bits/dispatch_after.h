...
inline auto dispatch_after(const chrono::duration<_Rep, _Period>& __rel_time, _CompletionToken&& __token)
{
  return (dispatch_at)(chrono::steady_clock::now() + __rel_time, forward<_CompletionToken>(__token));
}

template <class _Rep, class _Period, class _Executor, class _CompletionToken>
inline auto dispatch_after(const chrono::duration<_Rep, _Period>& __rel_time,
  const _Executor& __e, _CompletionToken&& __token,
    typename enable_if<is_executor<_Executor>::value>::type*)
{
  return (dispatch_at)(chrono::steady_clock::now() + __rel_time, __e, forward<_CompletionToken>(__token));
}
...
{
  return (dispatch_at)(chrono::steady_clock::now() + __rel_time, __c, forward<_CompletionToken>(__token));
}
