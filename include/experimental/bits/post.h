...
auto post(_CompletionToken&& __token)
{
  ...
  __completion_executor.post(std::move(__completion.handler), __completion_allocator);
  ...
  typename enable_if<is_executor<_Executor>::value>::type*)
{
  ...
  auto __completion_allocator(get_associated_allocator(__completion.handler));
  __completion_executor.post(__work_dispatcher<_Handler>(__completion.handler), __completion_allocator);
  return __completion.result.get();
}
