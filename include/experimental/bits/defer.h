...
auto defer(_CompletionToken&& __token)
{
  ...
  __completion_executor.defer(std::move(__completion.completion_handler), __completion_allocator);
  ...
  typename enable_if<is_executor<_Executor>::value>::type*)
{
  ...
  auto __completion_allocator(get_associated_allocator(__completion.completion_handler));
  __completion_executor.defer(__work_dispatcher<_Handler>(__completion.completion_handler), __completion_allocator);
  return __completion.result.get();
}
